...
const BarWidth_v = VelHistCanvas.width/200
const BarWidth = CanvasHist.width/NumEnergyLevelStat
const BarScale = CanvasHist.height/(NumPar*0.05)
...
function averages(totals) {
    ...
    console.table(aveVelHist)
    return {aveEnHist, aveVelHist}
}

function drawVelHist(avgs){
    VelContext.clearRect(0,0,200,200)
    VelContext.fillStyle = "steelblue"
    for (let j = 0; j < 200; j++) {
        const x = j*BarWidth_v
        const y = VelHistCanvas.height - avgs.aveVelHist[j]*BarScale;
        VelContext.fillRect(x,y,BarWidth_v,avgs.aveVelHist[j]*BarScale);
    }
}

function drawEnHist(avgs){
    HistContext.clearRect(0,0,500,200)
    HistContext.fillStyle = "steelblue"
    for (let j = 0; j < NumEnergyLevelStat; j++) {
        const x = j*BarWidth
        const y = CanvasHist.height - avgs.aveEnHist[j]*BarScale;
